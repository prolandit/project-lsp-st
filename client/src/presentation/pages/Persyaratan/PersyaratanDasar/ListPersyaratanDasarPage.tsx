...
const ListPersyaratanDasarPage = () => {
    ...
    const generateData = (skip: number, pageSize: number): Requirement[] => {
        const personDataList: Requirement[] = [];
        const endIndex = skip + pageSize;

        for (let i = skip; i < endIndex && i < 50; i++) {
            const basicReqs = new Requirement({
                id: i + 1,
    ...
    const data: Requirement[] = generateData(

...
import Muk from '../../../../data/models/Muk';
import Schema from '../../../../data/models/Schema';
import Button from '../../../components/Elements/Button';
import DataTable from '../../../components/Elements/DataTable';
import DeleteMukModal from '../../../components/Fragments/Persyaratan/Muk/DeleteMukModal';
...
const ListMukPage = () => {
            ...
            cell: ({ row }) => (
                <div className='flex flex-row items-center gap-4'>
                    <Button
                        ...
                        onClick={() =>
                            navigate(
                                `/persyaratan/materi-uji-kompetensi/edit/${row.original.id}`
                            )
                        }
                    >
                        <BiEdit
                    ...
                    <DeleteMukModal id={row.original.id} />
                </div>
            ),
        },
    ];

    const generateData = (skip: number, pageSize: number): Muk[] => {
        const personDataList: Muk[] = [];
        const endIndex = skip + pageSize;

        for (let i = skip; i < endIndex && i < 50; i++) {
            const basicReqs = new Muk({
                id: i + 1,
    ...
    const data: Muk[] = generateData(pageIndex * pageSize - pageSize, pageSize);
        ...
        <div className='flex flex-col mx-3 my-6 bg-white rounded-t-lg lg:mx-8'>
            <div className='flex flex-row items-center justify-between px-4 py-4 lg:px-6'>
                <span className='text-base font-semibold text-blue-600'>
                    Persyaratan / Master Data Materi Uji Kompetensi
                </span>
                <Button
                    type='button'
                    className='flex flex-row items-center gap-2'
                    onClick={() =>
                        navigate('/persyaratan/materi-uji-kompetensi/tambah')
                    }
                >
                    <FaPlus />
                    Tambah Materi Uji Komptensi
                </Button>
            </div>
            <hr />

...
export const userSchema = Yup.object().shape({
  ...
  signUpload: Yup.mixed<File>()
    .when("signExplanation", {
      is: (explanation: string) => !explanation || explanation.trim().length === 0,
      then: (schema) => schema.required("Tanda Tangan harus diisi"),
    })
    .test(
      "fileSize",
...
// Form Skema Admin
export const createSkemaSertifikat = Yup.object().shape({
  kodeSkema: Yup.string().required("Kode Skema tidak boleh kosong"),
  lisensiSkema: Yup.string()
    .oneOf(Constants.linsesiSkema.map((license) => license.value))
    .required("Lisensi Skema tidak boleh kosong"),
  namaSkema: Yup.string()
    // .min(6, "Password minimal 6 karakter")
    // .max(20, "Password maksimal 20 karakter")
    .required("Nama Skema tidak boleh kosong"),
  skkni: Yup.string(),
  year: Yup.number(),
  tipeform: Yup.string(),
});
